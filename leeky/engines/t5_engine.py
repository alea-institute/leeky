"""
This module implements the T5 engine for text completion
...
# model ID
# FLAN-T5 models like google/flan-t5-xl are also valid here
T5_MODEL_ID = "t5-3b"
...
class T5Engine(BaseEngine):
    """
    T5 text completion engine implementing the BaseEngine interface.
    """
    ...
    def get_valid_parameters(self) -> dict:
        ...
        for parameter_combination in product(*T5_VALID_PARAMETERS.values()):
            # convert to dict
            parameter_dict = dict(
                zip(T5_VALID_PARAMETERS.keys(), parameter_combination)
            )
        ...
        parameter_combination = self.rng.choice(
            list(
                combinations_with_replacement(
                    T5_VALID_PARAMETERS.values(), len(T5_VALID_PARAMETERS)
                )
            )
        )

        # convert to dict
        parameter_dict = dict(zip(T5_VALID_PARAMETERS.keys(), parameter_combination))
